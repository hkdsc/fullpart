...
@dataclass
class TransformerXLModelConfigSingleStage2(InstantiateConfig):
    _target: Type = field(default_factory=lambda: TransformerXLModelSingleStage2)
...
class TransformerXLModelSingleStage2(ModelMixin, ConfigMixin):
    _supports_gradient_checkpointing = True

    @register_to_config
    def __init__(
        self,
        transformer_config: TransformerXLModelConfigSingleStage2 = None,
    ):
        super().__init__()
        ...
        self.drop_global = transformer_config.drop_global
        self.slat_norm = transformer_config.slat_norm
        self.cornner_pos_emb = transformer_config.cornner_pos_emb
        self.id_emb = transformer_config.id_emb
        self.rotate_slat = transformer_config.rotate_slat
        ...
        from .transformer_voxel_part_stage2 import SLatFlowModel
        with open(os.path.join(self.ss_flow_weights_dir, 'slat_flow_img_dit_L_64l8p2_fp16.json'), 'r') as f:
        ...
        self.voxel_branch = SLatFlowModel(**voxel_branch_config['args'], global_block_id_list=self.global_block_list_voxel,
                                          cornner_pos_emb=self.cornner_pos_emb, id_emb=self.id_emb, rotate_slat=self.rotate_slat)
        self.voxel_branch.load_state_dict(load_file(os.path.join(self.ss_flow_weights_dir, 'slat_flow_img_dit_L_64l8p2_fp16.safetensors')),
                                          strict=False)
    ...
    def forward(
        ...
        assert np == (noisy_voxel_latent.coords[:, 0].max() + 1) // batch_size, f'np {np} mismatch'
        ...
        voxel_hidden_states, t_emb_voxel, cond_voxel, skips = self.voxel_branch.forward_pre_part(noisy_voxel_latent, timestep_voxel, img_conds, bboxes,
                                                                                          np, batch_size,)
        ...
        voxel_output = self.voxel_branch.forward_post_split(voxel_hidden_states, t_emb_voxel, skips, dtype=voxel_hidden_states.feats.dtype)
