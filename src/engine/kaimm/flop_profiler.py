...
class FlopsProfiler(object):
    ...
    def start_profile(self, ignore_list=None):
        ...
        def register_module_hooks(module, ignore_list):
            ...
            def start_mem_hook(module, input):
                get_accelerator().synchronize()
                module.__start_mem__ = torch.cuda.memory_allocated()
            ...
            if not hasattr(module, "__start_mem_hook_handle__"):
                module.__start_mem_hook_handle__ = module.register_forward_pre_hook(start_mem_hook)

            def end_time_hook(module, input, output):
                get_accelerator().synchronize()
                module.__duration__ += time.time() - module.__start_time__
            
            def end_mem_hook(module, input, output):
                get_accelerator().synchronize()
                module.__mem_change__ += torch.cuda.memory_allocated() - module.__start_mem__

            if not hasattr(module, "__end_time_hook_handle__"):
                module.__end_time_hook_handle__ = module.register_forward_hook(end_time_hook)

            if not hasattr(module, "__end_mem_hook_handle__"):
                module.__end_mem_hook_handle__ = module.register_forward_hook(end_mem_hook)
    ...
    def stop_profile(self):
        ...
        def remove_profile_attrs(module):
            ...
            if hasattr(module, "__start_mem_hook_handle__"):
                module.__start_mem_hook_handle__.remove()
                del module.__start_mem_hook_handle__
            if hasattr(module, "__end_mem_hook_handle__"):
                module.__end_mem_hook_handle__.remove()
                del module.__end_mem_hook_handle__
    ...
    def reset_profile(self):
        ...
        def add_or_reset_attrs(module):
            ...
            module.__start_mem__ = 0
            module.__mem_change__ = 0
    ...
    def end_profile(self):
        ...
        def remove_profile_attrs(module):
            ...
            if hasattr(module, "__start_mem__"):
                del module.__start_mem__
            if hasattr(module, "__mem_change__"):
                del module.__mem_change__
    ...
    def get_total_memory(self, as_string=False):
        ...
        Returns:
            The latency of the model forward pass.
        """
        total_memory = get_module_mem_changed(self.model)
        return memory_to_string(total_memory) if as_string else total_memory
    ...
    def print_model_profile(self, profile_step=1, module_depth=-1, top_modules=1, detailed=True, output_file=None):
        ...
        fwd_memory = self.get_total_memory()
        ...
        print('{:<60}  {:<8}'.format('fwd memory: ', fwd_memory // 1024**2))
        print('{:<60}  {:<8}'.format('fwd FLOPS per GPU = fwd flops per GPU / fwd latency: ',
                                     flops_to_string(total_flops / (fwd_latency + 0.000000001))))
        ...
        def flops_repr(module):
            ...
            memory = get_module_mem_changed(module)
            ...
            items.append(f"{memory_to_string(memory)} Memory")
    ...
    def print_model_aggregated_profile(self, module_depth=-1, top_modules=1):
        ...
        def walk_module(module, curr_depth, info):
            ...
            info[curr_depth][module.__class__.__name__][3] += get_module_mem_changed(module)
...
def memory_to_string(memory, units=None, precision=2):
    if units is None:
        if memory < 1024:
            return str(memory) + " B"
        if memory < 1024**2:
            return str(memory//1024) + " KB"
        if memory < 1024**3:
            return str(memory//1024**2) + " MB"
        if memory < 1024**4:
            return str(memory//1024**3) + " GB"


    #     if duration > 1:
    #         return str(round(duration, precision)) + " B"
    #     elif duration * 10**3 > 1:
    #         return str(round(duration * 10**3, precision)) + " KB"
    #     elif duration * 10**6 > 1:
    #         return str(round(duration * 10**6, precision)) + " us"
    #     else:
    #         return str(duration)
...
def get_module_mem_changed(module):
    mem_change = module.__mem_change__
    if mem_change == 0:  # e.g. ModuleList
        for m in module.children():
            mem_change += m.__mem_change__
    return mem_change
