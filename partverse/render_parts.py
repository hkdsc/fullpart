...
def render_parts(root_dir, ins_list, out_dir, num_views, use_random_views=False, elevation=30, save_mesh=False):
    _install_blender()
    fail_list = []
    empty_list = []
    for ins in tqdm(ins_list, total=len(ins_list), desc='Rendering parts'):
        ...
        for part_file in part_file_list:
            ...
            success = success and _render(
                part_file, save_part_dir, num_views, use_random_views=use_random_views, elevation=elevation, save_mesh=save_mesh)
        if not success:
...
if __name__ == '__main__':
    ...
    render_parts(root_dir, ins_list, out_dir, num_views=num_views, use_random_views=use_random_views, elevation=args.elevation, save_mesh=args.save_mesh)
